import { interview } from '@infra/libs/discord/webwooks';
...
export default {
  ...
  async handle({ data }) {
    ...
    schedule.scheduleJob(toSendAlert, async () => {
      ...
      const message = `Hey ${user.username.value
        } tudo bem? faço parte do time do HighPixel. Sou responsável pelas entrevistas e serei seu agenciador hoje. Antes de começarmos, gostaria de informar que sua entrevista ocorrerá em alguns minutos, precisamente às ${main
          .get('hour')
    ...
    schedule.scheduleJob(toFindNewStaff, async () => {
      ...
      if (
        ...
        interview.send(
          `<@&${process.env.BOT_DISCORD_ROLE_SUPPORT}> **${account.username.value}** está precisando de um orador clique em: **https://console.highpixel.com/console/appointments/${appointment.id}/${token.id}**`
        );
